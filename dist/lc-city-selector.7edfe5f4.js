...
var LcCity = function () {
  function LcCity(option) {
    _classCallCheck(this, LcCity);
  ...
  _createClass(LcCity, [{
    ...
    value: function getElement() {
      // 页面元素
      var lcCitySelect = document.getElementById('lcCitySelect' + this.randomNum);
      this.el = {
    ...
    value: function bindEvent() {
      ...
      var confirm = this.option.confirm,
          cancel = this.option.cancel,
          select = this.select,
          errMsg = '';

      // 取消
      this.el.lcCancel.addEventListener('click', function () {
        typeof cancel === 'function' && cancel();
        _this3.close();
      });
      // 确定
      this.el.lcConfirm.addEventListener('click', function () {
        // errMsg = '';
        for (var key in select) {
          if (!select[key]) {
            ...
            _this3.errorTip(errMsg);
            return;
          }
        };
        typeof confirm === 'function' && confirm(_this3.getSelect());
        _this3.close();
      });

      // 点击模态框也可以关闭
      this.el.lcCityMask.addEventListener('click', function () {
        _this3.close();
      });
    }
    ...
    key: 'getDataProvince',
    value: function getDataProvince() {
      var arr = [];
    ...
    key: 'renderProvince',
    value: function renderProvince(arr) {
      var html = this._renderList(arr);
      if (html) {
    ...
    key: 'renderDistrict',
    value: function renderDistrict(arr) {
      var html = this._renderList(arr);
      if (html) {
    ...
    key: 'getDataCity',
    value: function getDataCity(province) {
      ...
      for (var key in _data2.default) {
        if (key === province) {
          this.ShortData = _data2.default[key]; // 保存省份下城市的所有数据，选中区域用到
          for (var key in _data2.default[key]) {
            arr.push(key);
          };
          break;
        };
    ...
    key: 'getDataDistrict',
    value: function getDataDistrict(city) {
    ...
    key: 'onProvince',
    value: function onProvince() {
      var lcProvince = this.el.lcProvince,
      ...
      for (var i = 0, length = aLi.length; i < length; i++) {
        aLi[i].addEventListener('click', function () {
          ...
          for (var _i = 0, _length = aLi.length; _i < _length; _i++) {
            _util2.default.removeClass(aLi[_i], 'active');
          };
          _util2.default.addClass(this, 'active');
    ...
    value: function onCity() {
      ...
      for (var i = 0, length = aLi.length; i < length; i++) {
        aLi[i].addEventListener('click', function () {
          // 先删除所有的class, 再给当前元素添加class
          for (var _i2 = 0, _length2 = aLi.length; _i2 < _length2; _i2++) {
            _util2.default.removeClass(aLi[_i2], 'active');
          };
          _util2.default.addClass(this, 'active');
    ...
    key: 'onDistrict',
    value: function onDistrict() {
      var lcDistrict = this.el.lcDistrict,
      ...
      for (var i = 0, length = aLi.length; i < length; i++) {
        aLi[i].addEventListener('click', function () {
          // 先删除所有的class, 再给当前元素添加class
          for (var _i3 = 0, _length3 = aLi.length; _i3 < _length3; _i3++) {
            _util2.default.removeClass(aLi[_i3], 'active');
          };
          _util2.default.addClass(this, 'active');
    ...
    value: function renderNav() {
      ...
      var _line = function _line() {
        var lcNav = _this4.el.lcNav,
        ...
        setTimeout(function () {
          ...
          if (_this4.isFill) {
            _this4.fillCount.nav = true; // 只执行一次
        ...
        _this4.onNav(); // 给导航添加事件
      };

      if (this.isFill) {
        this.timeNav && clearTimeout(this.timeNav);
        // 防止重复渲染，已提高性能
        this.timeNav = setTimeout(function () {
          _line();
        }, 30);
      } else {
        _line();
      ...
      var _loop = function _loop(i) {
        aSpan[i].addEventListener('click', function () {
          ...
          for (var _i4 = 0; _i4 < length; _i4++) {
            _util2.default.removeClass(aSpan[_i4], 'active');
          }
          _util2.default.addClass(aSpan[i], 'active');
    ...
    key: 'errorTip',
    value: function errorTip(errMsg) {
      var lcErrorTip = this.el.lcErrorTip;

      lcErrorTip.innerHTML = errMsg;

      _util2.default.addClass(lcErrorTip, 'lc-show');
      setTimeout(function () {
        _util2.default.removeClass(lcErrorTip, 'lc-show');
      }, 1500);
    }
    ...
    key: 'addTranx',
    value: function addTranx() {
      var lcCenter = this.el.lcCenter;
      if (!_util2.default.hasClass(lcCenter, 'tranx')) {
        _util2.default.addClass(lcCenter, 'tranx');
      }
    }
    // 取消容器的偏移
    ...
    key: 'removeTranx',
    value: function removeTranx() {
      var lcCenter = this.el.lcCenter;
      if (_util2.default.hasClass(lcCenter, 'tranx')) {
        _util2.default.removeClass(lcCenter, 'tranx');
      }
    }
    ...
    value: function show(event) {
      // 判断是不是pc端打开
      if (_util2.default.getPageSize().windowWidth > 768 && event) {
        var offset = _util2.default.getOffset(event),
    ...
    value: function close() {
      var lcCitySelect = this.el.lcCitySelect;
      _util2.default.addClass(lcCitySelect, 'lc-close');

      setTimeout(function () {
        _util2.default.removeClass(lcCitySelect, 'lc-show');
        _util2.default.removeClass(lcCitySelect, 'lc-close');
      }, 300);
    }
    ...
    key: 'fillProvince',
    value: function fillProvince() {
      var province = this.option.data.province,
    ...
    key: 'fillCity',
    value: function fillCity() {
      var city = this.option.data.city,
    ...
    key: 'fillDistrict',
    value: function fillDistrict() {
      var district = this.option.data.district,
...
;
},{"./lc-city-selector.css":4,"./data.json":5,"./util.js":6}],2:[function(require,module,exports) {
'use strict';

var _lcCitySelector = require('./src/lc-city-selector.js');

var _lcCitySelector2 = _interopRequireDefault(_lcCitySelector);
...
// 有默认选项，进行回填
var defaultCity = new _lcCitySelector2.default({
  data: {
    province: '江西省',
    city: '南昌市',
    district: '青山湖区'
...
},{"./src/lc-city-selector.js":3}],9:[function(require,module,exports) {
...
//# sourceMappingURL=/lc-city-selector.7edfe5f4.map